...
function getRandomIntCoin(min, max) {
  min = Math.ceil(min);
  max = Math.floor(max);
...
// Flip Coin 3 = heads, 2 = tails
	Coin1 = getRandomIntCoin(2, 3);
	Coin2 = getRandomIntCoin(2, 3);
	Coin3 = getRandomIntCoin(2, 3);
}

var LineCastCoin = function(){
...
// Select a Marble
	...
	Chosen = getRandomIntMarble(1, Marbles);
}

var LineCastMarble = function(){
//This function creates the pictures of lines as broken or unbroken
//and changing or unchanging
...
function getRandomIntStalk(min, max) {
  min = Math.ceil(min);
  max = Math.floor(max);
...
WestPile = getRandomIntStalk(2, YarrowStalks); //Note that westpile split cannot be 1 or TotalYarrowStalks as both east and west piles cannot ever be 0
